...
ARGS=["--preserve-symlinks"]
if os.geteuid() == 0:
    ARGS.append("--run-as=root")
...
class ExtractionItem(object):
    ...
    def _check_firmware(self):
        ...
        """
        for module in binwalk.scan(self.item, "-y", "header", *ARGS,
    ...
    def _check_kernel(self):
        ...
        if not self.get_kernel_status():
            for module in binwalk.scan(self.item, "-y", "kernel", *ARGS,
                                       signature=True, quiet=True):
                for entry in module.results:
    ...
    def _check_recursive(self, fmt):
        ...
        desc = None
        # perform extraction
        for module in binwalk.scan(self.item, "-e", "-r", "-y", fmt, *ARGS,
                                   signature=True, quiet=True):
            for entry in module.results:
