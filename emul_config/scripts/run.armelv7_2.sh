...
IMAGE=`get_fs ${IID}`
KERNEL=`get_kernel "armelv7_2" ${IID}`
QEMU_MACHINE=`get_qemu_machine "armelv7_2"`
QEMU_ROOTFS=`get_qemu_disk "armelv7_2"`
...
QEMU_AUDIO_DRV=none qemu-system-arm -m 256 -M ${QEMU_MACHINE} -kernel ${KERNEL} -cpu cortex-a9 -drive if=sd,file=${IMAGE},format=raw -append "mem=256M rootwait fdyne_syscall=1 root=${QEMU_ROOTFS} console=ttyAMA0 nandsim.parts=64,64,64,64,64,64,64,64,64,64 ${QEMU_INIT} rw debug ignore_loglevel print-fatal-signals=1 FIRMAE_NET=${FIRMAE_NET} FIRMAE_NVRAM=${FIRMAE_NVRAM} FIRMAE_KERNEL=${FIRMAE_KERNEL} FIRMAE_ETC=${FIRMAE_ETC} user_debug=31" -serial file:${WORK_DIR}/qemu.initial.serial.log -serial unix:/tmp/qemu.${IID}.S1,server,nowait -monitor unix:/tmp/qemu.${IID},server,nowait -display none ${QEMU_NETWORK}
